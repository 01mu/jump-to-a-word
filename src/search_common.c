...
void search_complete(ShortcutJump *sj) {
    ...
    if ((sj->current_mode == JM_SEARCH || sj->current_mode == JM_SUBSTRING) &&
        sj->config_settings->text_after == TX_SELECT_TEXT_RANGE && !sj->range_is_set) {
    ...
    if (sj->current_mode == JM_SEARCH || sj->current_mode == JM_SUBSTRING) {
        ...
        if (sj->config_settings->text_after == TX_SELECT_TEXT_RANGE && sj->range_is_set) {
            if (pos > sj->range_first_pos) {
                ...
                scintilla_send_message(sj->sci, SCI_SETSEL, pos, sj->range_first_pos + sj->range_word_length);
            }
        }
    }
