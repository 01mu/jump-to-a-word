...
static void line_insert_set_positions(ShortcutJump *sj, GArray *lines_to_insert) {
...
static void clear_anchor_annotations(ScintillaObject *sci, GArray *anchors) {
    for (gint i = 0; i < anchors->len; i++) {
        Word word = g_array_index(anchors, Word, i);
...
static void insert_newline_chars(ShortcutJump *sj, GArray *anchors) {
    sj->added_new_line_insert = 0;
...
void line_insert_done_common(ShortcutJump *sj) {
    ...
    if (sj->replace_len <= 0) {
        replace = sj->buffer->str;
    } else {
        replace = sj->replace_cache->str;
    }
    ...
    scintilla_send_message(sj->sci, SCI_REPLACETARGET, -1, (sptr_t)replace);
